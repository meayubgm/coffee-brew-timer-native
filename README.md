...
テストランナーは未導入。型安全性は `npx tsc --noEmit`、Web ビルドは `npx expo export --platform web` で確認する。

ブラウザでの動作確認は **Playwright MCP（`mcp__playwright__*`）を優先**して行う。`npx expo start` で Web 版（`localhost:8081`）を起動し、アクセシビリティスナップショットベースで要素を特定して操作・検証する。

---

## Web版デプロイ

ホスティングは **Vercel**（無料サブドメイン `*.vercel.app`）。CLI からの手動デプロイで運用する（GitHub 連携の自動デプロイは未設定）。

`vercel.json` で Vercel クラウド側のビルド（`expo export --platform web`）と、SPA 用の rewrite（全パスを `/` にフォールバック）を定義している。rewrite が無いと `/timer` などのディープリンク直アクセス・リロードで 404 になる。

```bash
# 初回のみ: Vercel にログイン（ブラウザ認証）
npx vercel login

# ローカルでビルド検証（任意）
npm run build:web        # = expo export --platform web

# プレビュー公開（初回は新規プロジェクト作成の対話あり）
npx vercel
...
> 現在は `public/index.html` の `noindex, nofollow` により検索エンジン非公開（ソフトローンチ）。正式公開時にこのメタを外す。
